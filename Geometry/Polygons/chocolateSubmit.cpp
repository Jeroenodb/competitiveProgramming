...
int main() {
    ...
    vector<pt> vs(n);
    for(int i=0;i<n;++i) {
        vs[i] = pts[(i+1)%n]-pts[i];
    }
    ofstream optimal("optimal.out");
    for(int i=0;i<n;++i) {
        for(int  j = i+1;j<n;++j) {
            auto a = arg(vs[i]), b = arg(vs[j]);
            auto c = a+b/2;
            while(c<0) c+=PI;
            while(c>PI) c-=PI;
            optimal << c << ' ' << calc(c) << '\n';
